...
namespace Clinic
{
    public partial class home : Page
    {
        ...
        $"<label>Name:</label><input type='text' name='updateName' value='{patientRow["Name"]?.ToString() ?? string.Empty}'/><br/>" +
        $"<label>Address:</label><input type='text' name='updateAddress' value='{patientRow["Address"]?.ToString() ?? string.Empty}'/><br/>" +
        $"<label>Birth Date:</label><input type='date' name='updateBirthDate' value='{((DateTime)patientRow["BirthDate"]).ToString("yyyy-MM-dd")}'/><br/>" +
        $"<label>Phone:</label><input type='text' name='updatePhone' value='{patientRow["Phone"]?.ToString() ?? string.Empty}'/><br/>" +
        $"<label>Cell Phone:</label><input type='text' name='updateCellPhone' value='{patientRow["CellPhone"]?.ToString() ?? string.Empty}'/><br/>";
            inputsAdd += "<h3>Corona Details:</h3>" +
                 $"<label>Date of Illness:</label><input type='date' name='coronaDateOfIllness' value = '{((DateTime)coronaRow?["Date_Of_Illness"]).ToString("yyyy-MM-dd")}' /><br/> " +
                 $"<label>Date of Recovery:</label><input type='date' name='coronaDateOfRecovery' value = '{((DateTime)coronaRow?["Date_Of_Recovery"]).ToString("yyyy-MM-dd")}'/><br/> ";
                inputsAdd += "<h3>Vaccination Details:</h3>" +
                     $"<label>Code:</label><input type='text' name='vaccineCode' value='{vaccinatedRow?["Code_V"]}'/><br/>" +
                     $"<label>Date:</label><input type='date' name='vaccineDate'  value = '{((DateTime)vaccinatedRow?["Date_V"]).ToString("yyyy-MM-dd")}' /><br/> " +
